        ...
        { //XXX FIXME optimize
            ...
            if ((unsigned) src_x < width)
            {
                if ((unsigned) src_y < height)
                {
                    ...
                    dst[y * stride + x] = ((src[index] * (s - frac_x) + src[index + 1] * frac_x) * (s - frac_y) + (src[index + stride] * (s - frac_x) + src[index + stride + 1] * frac_x) * frac_y + r) >> (shift * 2);
                }
                else
                {
                    index = src_x + av_clip(src_y, 0, height) * stride;
                    dst[y * stride + x] = ((src[index] * (s - frac_x) + src[index + 1] * frac_x) * s + r) >> (shift * 2);
                }
            }
            else
            {
                if ((unsigned) src_y < height)
                {
                    ...
                    dst[y * stride + x] = ((src[index] * (s - frac_y) + src[index + stride] * frac_y) * s + r) >> (shift * 2);
                }
                else
                {
        ...
        if (y + 1 < h)
        {
            for (x = 0; x < 15; x++)
            {
                score2 += FFABS( s1[x ] - s1[x +stride]
                    - s1[x+1] + s1[x+1+stride]) - FFABS( s2[x ] - s2[x +stride]
...
static int hadamard8_diff8x8_c( /*MpegEncContext*/void *s, uint8_t *dst, uint8_t *src, int stride, int h )
{
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        sum += BUTTERFLYA(temp[8*0+i], temp[8*4+i]) + BUTTERFLYA(temp[8*1+i], temp[8*5+i]) + BUTTERFLYA(temp[8*2+i], temp[8*6+i]) + BUTTERFLYA(temp[8*3+i], temp[8*7+i]);
    }
#if 0
    static int maxi=0;
    if(sum>maxi)
...
static int vsad_intra16_c( /*MpegEncContext*/void *c, uint8_t *s, uint8_t *dummy, int stride, int h )
{
    ...
    for (y = 1; y < h; y++)
    {
        for (x = 0; x < 16; x += 4)
        {
            score += FFABS(s[x ] - s[x +stride]) + FFABS(s[x+1] - s[x+1+stride]) + FFABS(s[x+2] - s[x+2+stride]) + FFABS(s[x+3] - s[x+3+stride]);
        }
        s += stride;
    }
...
static int vsse_intra16_c( /*MpegEncContext*/void *c, uint8_t *s, uint8_t *dummy, int stride, int h )
{
    ...
    for (y = 1; y < h; y++)
    {
        for (x = 0; x < 16; x += 4)
        {
            score += SQ(s[x ] - s[x +stride]) + SQ(s[x+1] - s[x+1+stride]) + SQ(s[x+2] - s[x+2+stride]) + SQ(s[x+3] - s[x+3+stride]);
        }
        s += stride;
    }
