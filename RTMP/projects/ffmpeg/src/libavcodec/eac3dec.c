...
typedef enum
{
    EAC3_GAQ_NO = 0, EAC3_GAQ_12, EAC3_GAQ_14, EAC3_GAQ_124
} EAC3GaqMode;
...
int ff_eac3_parse_header( AC3DecodeContext *s )
{
    ...
    if (s->frame_type == EAC3_FRAME_TYPE_AC3_CONVERT && (s->num_blocks == 6 || get_bits1(gbc)))
    {
    ...
    if (s->frame_type == EAC3_FRAME_TYPE_INDEPENDENT && (s->num_blocks == 6 || get_bits1(gbc)))
    {
    ...
    if (parse_aht_info)
    {
        ...
        for (ch = (num_cpl_blocks != 6); ch <= s->channels; ch++)
        {
            ...
            for (blk = 1; blk < 6; blk++)
            {
                if ((s->exp_strategy[blk][ch] != EXP_REUSE) || (!ch && s->cpl_strategy_exists[blk]))
                {
                    use_aht = 0;
                    break;
                }
