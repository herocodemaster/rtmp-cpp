...
static inline void mpeg4_encode_block( MpegEncContext * s, DCTELEM * block, int n, int dc, uint8_t *scan_table, PutBitContext *dc_pb, PutBitContext *ac_pb );
...
{
    av_log(s->avctx, AV_LOG_DEBUG, "qp:%d %c size:%d rnd:%d%s%s%s%s%s%s%s%s%s %d/%d\n", s->qscale, av_get_pict_type_char(s->pict_type), s->gb.size_in_bits, 1 - s->no_rounding, s->obmc ? " AP" : "", s->umvplus ? " UMV" : "", s->h263_long_vectors ? " LONG" : "", s->h263_plus ? " +" : "", s->h263_aic ? " AIC" : "", s->alt_inter_vlc ? " AIV" : "", s->modified_quant ? " MQ" : "", s->loop_filter ? " LOOP" : "", s->h263_slice_structured ? " SS" : "", s->avctx->time_base.den, s->avctx->time_base.num);
}
...
void ff_h263_update_motion_val( MpegEncContext * s )
{
    ...
    if (s->mv_type != MV_TYPE_8X8)
    {
        ...
        else /*if (s->mv_type == MV_TYPE_FIELD)*/
        {
            ...
            s->current_picture.ref_index[0][xy] = s->current_picture.ref_index[0][xy + 1] = s->field_select[0][0];
            s->current_picture.ref_index[0][xy + wrap] = s->current_picture.ref_index[0][xy + wrap + 1] = s->field_select[0][1];
        }
...
static inline void ff_h263_encode_motion_vector( MpegEncContext * s, int x, int y, int f_code )
{
    ...
    {
        skip_put_bits(&s->pb, h263_get_motion_length(s, x, f_code) + h263_get_motion_length(s, y, f_code));
    }
    else
    {
...
static inline int get_p_cbp( MpegEncContext * s, DCTELEM block[6][64], int motion_x, int motion_y )
{
    LogStr("Init");
    ...
    if (s->flags & CODEC_FLAG_CBP_RD)
    {
        ...
        for (i = 0; i < 4; i++)
        {
            ...
            if (i & 1)
                score += s->coded_score[5];
            if (i & 2)
                score += s->coded_score[4];
        ...
        for (i = 0; i < 16; i++)
        {
            ...
            if (i & 1)
                score += s->coded_score[3];
            if (i & 2)
                score += s->coded_score[2];
            if (i & 4)
                score += s->coded_score[1];
            if (i & 8)
                score += s->coded_score[0];
...
static inline int get_b_cbp( MpegEncContext * s, DCTELEM block[6][64], int motion_x, int motion_y, int mb_type )
{
    LogStr("Init");
...
static inline void mpeg4_encode_blocks( MpegEncContext * s, DCTELEM block[6][64], int intra_dc[6], uint8_t **scan_table, PutBitContext *dc_pb, PutBitContext *ac_pb )
{
    LogStr("Init");
...
void mpeg4_encode_mb( MpegEncContext * s, DCTELEM block[6][64], int motion_x, int motion_y )
{
    ...
    if (!s->mb_intra)
    {
        ...
        if (s->pict_type == FF_B_TYPE)
        {
            ...
            { //FIXME avoid DCT & ...
                s->skip_count++;
                s->mv[0][0][0] = s->mv[0][0][1] = s->mv[1][0][0] = s->mv[1][0][1] = 0;
            ...
            else
            {
                ...
                if (s->mv_type != MV_TYPE_FIELD)
                {
                    ...
                    {
                        ff_h263_encode_motion_vector(s, s->mv[0][0][0] - s->last_mv[0][0][0], s->mv[0][0][1] - s->last_mv[0][0][1], s->f_code);
                    ...
                    {
                        ff_h263_encode_motion_vector(s, s->mv[1][0][0] - s->last_mv[1][0][0], s->mv[1][0][1] - s->last_mv[1][0][1], s->b_code);
                ...
                else
                {
                    ...
                    if (s->mv_dir & MV_DIR_FORWARD)
                    {
                        for (i = 0; i < 2; i++)
                        {
                            ff_h263_encode_motion_vector(s, s->mv[0][i][0] - s->last_mv[0][i][0], s->mv[0][i][1] - s->last_mv[0][i][1] / 2, s->f_code);
                    ...
                    if (s->mv_dir & MV_DIR_BACKWARD)
                    {
                        for (i = 0; i < 2; i++)
                        {
                            ff_h263_encode_motion_vector(s, s->mv[1][i][0] - s->last_mv[1][i][0], s->mv[1][i][1] - s->last_mv[1][i][1] / 2, s->b_code);
        ...
        { /* s->pict_type==FF_B_TYPE */
            ...
            if ((cbp | motion_x | motion_y | s->dquant) == 0 && s->mv_type == MV_TYPE_16X16)
            {
                ...
                if (s->max_b_frames > 0)
                {
                    ...
                    for (i = 0; i < s->max_b_frames; i++)
                    {
                        ...
                        if (pic == NULL || pic->pict_type != FF_B_TYPE)
                            break;
            ...
            if (s->mv_type == MV_TYPE_16X16)
            {
                if (s->dquant)
                    cbpc += 8;
                put_bits(&s->pb, inter_MCBPC_bits[cbpc], inter_MCBPC_code[cbpc]);
                ...
                ff_h263_encode_motion_vector(s, motion_x - pred_x, motion_y - pred_y, s->f_code);
            }
            else if (s->mv_type == MV_TYPE_FIELD)
            {
                if (s->dquant)
                ...
                ff_h263_encode_motion_vector(s, s->mv[0][0][0] - pred_x, s->mv[0][0][1] - pred_y, s->f_code);
                ff_h263_encode_motion_vector(s, s->mv[0][1][0] - pred_x, s->mv[0][1][1] - pred_y, s->f_code);
            }
            else
            {
                assert(s->mv_type==MV_TYPE_8X8);
                put_bits(&s->pb, inter_MCBPC_bits[cbpc + 16], inter_MCBPC_code[cbpc + 16]);
                put_bits(pb2, cbpy_tab[cbpy][1], cbpy_tab[cbpy][0]);
                ...
                for (i = 0; i < 4; i++)
                {
                    ...
                    ff_h263_encode_motion_vector(s, s->current_picture.motion_val[0][s->block_index[i]][0] - pred_x, s->current_picture.motion_val[0][s->block_index[i]][1] - pred_y, s->f_code);
                }
            }
    ...
    else
    {
        ...
        if (s->pict_type == FF_I_TYPE)
        {
            if (s->dquant)
                cbpc += 4;
            put_bits(&s->pb, intra_MCBPC_bits[cbpc], intra_MCBPC_code[cbpc]);
        }
        else
        {
            if (s->dquant)
                cbpc += 8;
            put_bits(&s->pb, 1, 0); /* mb coded */
            put_bits(&s->pb, inter_MCBPC_bits[cbpc + 4], inter_MCBPC_code[cbpc + 4]);
        }
...
void mpeg4_encode_picture_header( MpegEncContext * s, int picture_number )
{
    ...
    if (s->pict_type == FF_P_TYPE || (s->pict_type == FF_S_TYPE && s->vol_sprite_usage == GMC_SPRITE))
    {
        put_bits(&s->pb, 1, s->no_rounding); /* rounding type */
    }
 ...
 * @param n block index (0-3 are luma, 4-5 are chroma)
 */
static inline void mpeg4_encode_block( MpegEncContext * s, DCTELEM * block, int n, int intra_dc, uint8_t *scan_table, PutBitContext *dc_pb, PutBitContext *ac_pb )
{
    LogStr("Init");
...
static void mpeg4_decode_sprite_trajectory( MpegEncContext * s, GetBitContext *gb )
{
    ...
    while ((1 << alpha) < w)
        alpha++;
    while ((1 << beta) < h)
        beta++; // there seems to be a typo in the mpeg4 std for the definition of w' and h'
    w2 = 1 << alpha;
    h2 = 1 << beta;
    ...
    switch (s->num_sprite_warping_points)
    {
        ...
        case 1: //GMC only
            ...
            break;
        case 2:
            s->sprite_offset[0][0] = (sprite_ref[0][0] << (alpha + rho)) + (-r * sprite_ref[0][0] + virtual_ref[0][0]) * (-vop_ref[0][0]) + (r * sprite_ref[0][1] - virtual_ref[0][1]) * (-vop_ref[0][1]) + (1 << (alpha + rho - 1));
    ...
    if (s->shape != BIN_ONLY_SHAPE)
    {
        ...
        { /* not_8_bit */
            ...
            if (s->quant_precision != 5)
                av_log(s->avctx, AV_LOG_ERROR, "quant precision %d\n", s->quant_precision);
        }
        else
        {
            s->quant_precision = 5;
        ...
        if (!get_bits1(gb))
            av_log(s->avctx, AV_LOG_ERROR, "Complexity estimation not supported\n");
        ...
        if (vo_ver_id != 1)
        {
            ...
            if (s->reduced_res_vop)
                av_log(s->avctx, AV_LOG_ERROR, "reduced resolution VOP not supported\n");
        }
        else
        {
            s->new_pred = 0;
...
static int decode_vop_header( MpegEncContext *s, GetBitContext *gb )
{
    ...
    if (s->time_increment_bits == 0 || !(show_bits(gb, s->time_increment_bits + 1) & 1))
    {
        ...
        {
            if (show_bits(gb, s->time_increment_bits + 1) & 1)
                break;
        }
    ...
    if (s->shape != BIN_ONLY_SHAPE && (s->pict_type == FF_P_TYPE || (s->pict_type == FF_S_TYPE && s->vol_sprite_usage == GMC_SPRITE)))
    {
