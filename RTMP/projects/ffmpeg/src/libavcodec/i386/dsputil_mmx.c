...
static void gmc_mmx( uint8_t *dst, uint8_t *src, int stride, int h, int ox, int oy, int dxx, int dxy, int dyx, int dyy, int shift, int r, int width, int height )
{
    ...
    ((ox ^ (ox + dxw)) | (ox ^ (ox + dxh)) | (ox ^ (ox + dxw + dxh)) | (oy ^ (oy + dyw)) | (oy ^ (oy + dyh)) | (oy ^ (oy + dyw + dyh))) >> (16 + shift)
    ...
    src += ix + iy * stride;
    if ((unsigned) ix >= width - w || (unsigned) iy >= height - h)
    {
...
extern void ff_snow_inner_add_yblock_sse2( const uint8_t *obmc, const int obmc_stride, uint8_t * * block, int b_w, int b_h, int src_x, int src_y, int src_stride, slice_buffer * sb, int add, uint8_t * dst8 );
extern void ff_snow_inner_add_yblock_mmx( const uint8_t *obmc, const int obmc_stride, uint8_t * * block, int b_w, int b_h, int src_x, int src_y, int src_stride, slice_buffer * sb, int add, uint8_t * dst8 );
...
void dsputil_init_mmx( DSPContext* c, AVCodecContext *avctx )
{
    ...
    if (mm_flags & MM_MMX)
    {
        ...
        if (avctx->lowres == 0)
        {
            ...
            else if ((ENABLE_VP3_DECODER || ENABLE_VP5_DECODER || ENABLE_VP6_DECODER) && idct_algo == FF_IDCT_VP3 && avctx->codec->id != CODEC_ID_THEORA && !(avctx->flags & CODEC_FLAG_BITEXACT))
            {
                if (mm_flags & MM_SSE2)
                {
...
#define SET_HPEL_FUNCS(PFX, IDX, SIZE, CPU) \
        ...
        c->h264_idct_dc_add = c->h264_idct_add = ff_h264_idct_add_mmx;
        c->h264_idct8_dc_add = c->h264_idct8_add = ff_h264_idct8_add_mmx;
        if (mm_flags & MM_SSE2)
